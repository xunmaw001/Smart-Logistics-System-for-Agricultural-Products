    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(forum !=null){
            ...
            //级联表 农产品供应商
            //级联表
            GongyingshangEntity gongyingshang = gongyingshangService.selectById(forum.getGongyingshangId());
            if(gongyingshang != null){
            BeanUtils.copyProperties( gongyingshang , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "gongyingshangId"
, "yonghuId"
, "usersId"
, "wuliugongsiId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            if(yonghu != null){
            BeanUtils.copyProperties( yonghu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "gongyingshangId"
, "yonghuId"
, "usersId"
, "wuliugongsiId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 农产品物流公司
            //级联表
            WuliugongsiEntity wuliugongsi = wuliugongsiService.selectById(forum.getWuliugongsiId());
            if(wuliugongsi != null){
            BeanUtils.copyProperties( wuliugongsi , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "gongyingshangId"
, "yonghuId"
, "usersId"
, "wuliugongsiId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setWuliugongsiId(wuliugongsi.getId());
            }
            //级联表 管理员
            //管理员表做额外的处理
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setGongyingshangId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
            ...
            forum.setWuliugongsiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            .eq("forum_name", forum.getForumName())
            .eq("gongyingshang_id", forum.getGongyingshangId())
            .eq("wuliugongsi_id", forum.getWuliugongsiId())
...
//                            forumEntity.setGongyingshangId(Integer.valueOf(data.get(0)));   //农产品供应商 要改的
//                            forumEntity.setWuliugongsiId(Integer.valueOf(data.get(0)));   //农产品物流公司 要改的
        ...
        ForumEntity forum = forumService.selectById(id);
            if(forum !=null){
                ...
                //级联表
                    WuliugongsiEntity wuliugongsi = wuliugongsiService.selectById(forum.getWuliugongsiId());
                if(wuliugongsi != null){
                    BeanUtils.copyProperties( wuliugongsi , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setWuliugongsiId(wuliugongsi.getId());
                }
                    UsersEntity users = usersService.selectById(forum.getUsersId());
                if(users != null){
