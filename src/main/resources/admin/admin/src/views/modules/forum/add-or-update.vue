...
<script>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                   ...
                   gongyingshangId: [
                              { required: true, message: '农产品供应商不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   wuliugongsiId: [
                              { required: true, message: '农产品物流公司不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
        ...
        created() {
         ...
         this.$http({
             url: `wuliugongsi/page?page=1&limit=100`,
             method: "get"
         }).then(({ data }) => {
             if (data && data.code === 0) {
                this.wuliugongsiOptions = data.data.list;
            }
         });
         this.$http({
        ...
        methods: {
            ...
            wuliugongsiChange(id){
                this.$http({
                    url: `wuliugongsi/info/`+id,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.wuliugongsiForm = data.data;
                    }
                });
            },
            yonghuChange(id){
            ...
            info(id) {
                ...
                _this.$http({
                    ...
                    if (data && data.code === 0) {
                        ...
                        _this.wuliugongsiChange(data.data.wuliugongsiId)
